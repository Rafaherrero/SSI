...
aes::aes(std::vector<vector<unsigned char>> cl, std::vector<vector<unsigned char>> tx, unsigned int np):
clave_(cl),
texto_(tx),
texto_cf_(tx),
...
aes::aes(std::vector<vector<unsigned char>> cl, std::vector<vector<unsigned char>> tx, std::vector<vector<unsigned char>> cl_ant):
clave_(cl),
texto_(tx),
texto_cf_(tx),
clave_exp_ (std::vector<vector<vector<unsigned char>>>(11, vector<vector<unsigned char>>(4, vector<unsigned char>(4)))),
clave_anterior_(cl_ant),
iter_(0),
no_imprimir_(1)
{
	addRoundKey_mod();
	encriptar();
	clave_anterior_ = texto_cf_;
}

void aes::addRoundKey_mod(void){
	for (int i=0; i<4; i++){
		for (int j=0; j<4; j++){
			texto_cf_[j][i] = texto_cf_[j][i] ^ clave_anterior_[j][i];
		}
	}
}

std::vector<vector<unsigned char>> aes::get_clave_anterior(void){
	return clave_anterior_;
}

std::vector<vector<unsigned char>> aes::get_texto_encriptado(void){
	return texto_cf_;
}
